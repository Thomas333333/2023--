...
//    }
    ...
    public List<QApair> parseJson(String json_file_path) throws IOException {
        File jsonFile = new File(json_file_path);
        String jsonStr="";
        String jsonData=null;
        List<QApair> questionList=null;
        try {
            jsonData = new String(Files.readAllBytes(Paths.get(json_file_path)), StandardCharsets.UTF_8);
            questionList = JSON.parseArray(jsonData, QApair.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//
//        questionList = JSON.parseObject(jsonStr, new TypeReference<List<QApair>>(){}, Feature.IgnoreNotMatch);
